...
def CreateAnimator():
    DB=database.DataLoarder().create_database()
    path_model=DB.path_root+"/model/"
    cuda = torch.device('cuda' if torch.cuda.is_available() else "cpu")
    poser = MorphRotateCombinePoser256Param6(
        morph_module_spec=FaceMorpherSpec(),
        morph_module_file_name=path_model+"/data/face_morpher.pt",
        rotate_module_spec=TwoAlgoFaceRotatorSpec(),
        rotate_module_file_name=path_model+"/data/two_algo_face_rotator.pt",
        combine_module_spec=CombinerSpec(),
        combine_module_file_name=path_model+"/data/combiner.pt",
        device=cuda)
    ...
    landmark_locator = dlib.shape_predictor(path_model+"\data\shape_predictor_68_face_landmarks.dat")
